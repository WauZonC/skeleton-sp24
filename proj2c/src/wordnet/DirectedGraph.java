...
public class DirectedGraph {
    ...
    public void makeEdge(int fromLabel, int toLabel) {
        ...
        nodes.get(toLabel).ancestors.add(fromLabel);
    }
    ...
    public TreeSet<String> reachableAncestors(List<Integer> labels) {
        boolean[] markedList = new boolean[nodes.size()];
        TreeSet<String> returnSet = new TreeSet<>();
        for (int label : labels) {
            reachableAncestors(label, markedList, returnSet);
        }
        return returnSet;
    }

    private void reachableAncestors(int label, boolean[] markedList, TreeSet<String> set) {
        markedList[label] = true;
        set.addAll(nodes.get(label).synsets);
        for (int ancestor : nodes.get(label).ancestors) {
            if (!markedList[ancestor]) {
                reachableAncestors(ancestor, markedList, set);
            }
        }
    }
